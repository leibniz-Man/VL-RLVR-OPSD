...
class AlgorithmConfig:
    ...
    # ── RLSD hyperparameters (arXiv:2604.03128) ───────────────────────────
    rlsd_lambda_init: float = 0.5
    """Initial mixing coefficient λ. Controls how much self-distillation
    credit reweighting is applied. Linearly decayed to 0 over
    rlsd_lambda_decay_steps. At 0, RLSD reduces to standard GRPO."""

    rlsd_lambda_decay_steps: int = 50
    """Number of training steps over which λ is linearly decayed from
    rlsd_lambda_init to 0. Paper uses 50 steps (out of 200 total)."""

    rlsd_epsilon_w: float = 0.2
    """Clip bound for per-token evidence weights wt ∈ [1−εw, 1+εw].
    Acts as a trust-region for credit redistribution. Paper uses 0.2."""

    rlsd_teacher_sync_freq: int = 10
    """How many training steps between synchronizing teacher weights with
    the current student. Paper uses 10. Set to 1 to always use the
    latest student weights (online teacher, simpler but slightly less
    stable)."""

    # CEPO parameters
    use_cepo: bool = False
    """enable Contrastive Evidence Policy Optimization token weighting"""
    cepo_lambda_init: float = 1.0
    """initial mixing coefficient λ (linearly decayed to 0 over cepo_warmup_steps)"""
    cepo_warmup_steps: int = 100
    """number of gradient steps over which λ decays from cepo_lambda_init to 0"""
    cepo_eps_w: float = 0.2
    """clip range for token weights: clip(w_t, 1-eps_w, 1+eps_w)"""
    cepo_use_cot_teacher: bool = False
    """Condition teacher on reasoning prefix from best/worst rollouts
    instead of bare answer tokens."""
    cepo_cot_prefix_len: int = 200
    """Number of leading response tokens used as teacher prefix.
    Only active when cepo_use_cot_teacher=True."""
    cepo_lambda_schedule: str = "linear"
    """Schedule for decaying cepo_lambda: 'linear' or 'constant'"""
    cepo_teacher_mode: str = "answer"
    """CEPO teacher contrast source: 'answer' keeps original CEPO; 'image_mask' contrasts original vs masked image."""
    cepo_mask_mode: str = "black"
    """Masking mode for image_mask teacher. Currently supports 'black'."""
    use_ecvc: bool = False
    """Enable entropy-controlled visual credit on top of the image-mask CEPO teachers."""
    ecvc_entropy_alpha: float = 5.0
    ecvc_margin_pos_z: float = 0.5
    ecvc_margin_neg_z: float = 0.5
    ecvc_gamma: float = 0.5
    ecvc_normalize_entropy_gap: bool = True
    ecvc_calibration_enabled: bool = True
    ecvc_calibration_num_samples: int = 128
    ecvc_calibration_eps: float = 1e-6
