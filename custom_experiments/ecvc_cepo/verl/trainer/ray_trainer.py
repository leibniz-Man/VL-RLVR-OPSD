...
class RayPPOTrainer:
    ...
    def _build_cepo_image_mask_teacher_batch(self, batch: DataProto) -> DataProto:
        """Build CEPO teacher inputs that contrast original image vs masked image.

        Unlike the original CEPO answer-conditioned teacher, both teachers use the
        same text sequence [prompt, response]. The positive teacher sees the
        original image, and the negative teacher sees a same-size masked image.
        """
        ...
        if "multi_modal_data" not in batch.non_tensor_batch:
            raise RuntimeError("CEPO image_mask teacher requires multi_modal_data in the rollout batch.")

        mask_mode = getattr(self.config.algorithm, "cepo_mask_mode", "black")
        if mask_mode != "black":
            raise ValueError(f"Unsupported CEPO image mask mode: {mask_mode}")
        ...
        def _black_image_like(image):
            loaded = _load_image(image)
            return Image.new("RGB", loaded.size, (0, 0, 0))

        def _mask_multi_modal_data(multi_modal_data):
            masked = dict(multi_modal_data)
            if "images" not in masked:
                raise RuntimeError("CEPO image_mask teacher currently supports image samples only.")
            masked["images"] = [_black_image_like(image) for image in masked["images"]]
            return masked

        pos_multi_modal_data = batch.non_tensor_batch["multi_modal_data"]
        neg_multi_modal_data = np.array(
            [_mask_multi_modal_data(multi_modal_data) for multi_modal_data in pos_multi_modal_data],
            dtype=object,
        )

        teacher_batch = DataProto.from_dict(
            tensors={
                "pos_input_ids": input_ids.clone(),
                "pos_attention_mask": attention_mask.clone(),
                "pos_position_ids": position_ids.clone(),
                "neg_input_ids": input_ids.clone(),
                "neg_attention_mask": attention_mask.clone(),
                "neg_position_ids": position_ids.clone(),
                "responses": responses,
            },
            non_tensors={
                "uid": batch.non_tensor_batch.get("uid", np.array([""] * bs, dtype=object)),
                "pos_multi_modal_data": pos_multi_modal_data,
                "neg_multi_modal_data": neg_multi_modal_data,
            },
            meta_info={**batch.meta_info, "temperature": self.config.worker.rollout.temperature},
        )
        return teacher_batch
    ...
    def fit(self):
        ...
        self._ecvc_entropy_gap_std = None
                ...
                with timer("adv", timing_raw):
                    ...
                    if self.config.algorithm.use_cepo and self._cepo_lambda > 0:
                        with timer("cepo_teacher", timing_raw):
                            cepo_teacher_mode = getattr(self.config.algorithm, "cepo_teacher_mode", "answer")
                            if cepo_teacher_mode == "image_mask":
                                teacher_batch = self._build_cepo_image_mask_teacher_batch(batch)
                            elif cepo_teacher_mode == "answer":
                                teacher_batch = self._build_cepo_teacher_batch(batch)
                            else:
                                raise ValueError(f"Unsupported CEPO teacher mode: {cepo_teacher_mode}")
                        ...
                        pos_entropy = teacher_output.batch["pos_teacher_entropies"]
                        neg_entropy = teacher_output.batch["neg_teacher_entropies"]
                        ...
                        if self.config.algorithm.use_ecvc:
                            _entropy_gap = (neg_entropy - pos_entropy).float()
                            if self._ecvc_entropy_gap_std is None:
                                if (
                                    self.config.algorithm.ecvc_calibration_enabled
                                    and self.config.algorithm.ecvc_normalize_entropy_gap
                                ):
                                    _calib_rows = min(
                                        int(self.config.algorithm.ecvc_calibration_num_samples),
                                        _entropy_gap.size(0),
                                    )
                                    _calib_values = _entropy_gap[:_calib_rows][_mask[:_calib_rows]]
                                    if _calib_values.numel() < 2:
                                        raise RuntimeError("ECVC calibration requires at least two valid response tokens.")
                                    self._ecvc_entropy_gap_std = max(
                                        float(_calib_values.std(unbiased=False).item()),
                                        float(self.config.algorithm.ecvc_calibration_eps),
                                    )
                                else:
                                    self._ecvc_entropy_gap_std = 1.0
                                print(
                                    f"[ECVC calibration] rows={min(int(self.config.algorithm.ecvc_calibration_num_samples), _entropy_gap.size(0))}, "
                                    f"sigma_U_init={self._ecvc_entropy_gap_std:.8f}"
                                )

                            batch.batch["advantages"], _ecvc_diag = apply_ecvc_cepo_weighting(
                                advantages=_adv_pre,
                                response_mask=batch.batch["response_mask"],
                                vis_teacher_log_probs=pos_lp,
                                mask_teacher_log_probs=neg_lp,
                                vis_teacher_entropies=pos_entropy,
                                mask_teacher_entropies=neg_entropy,
                                entropy_gap_std=self._ecvc_entropy_gap_std,
                                lam=_cepo_lambda_used,
                                eps_w=self.config.algorithm.cepo_eps_w,
                                alpha=self.config.algorithm.ecvc_entropy_alpha,
                                margin_pos_z=self.config.algorithm.ecvc_margin_pos_z,
                                margin_neg_z=self.config.algorithm.ecvc_margin_neg_z,
                                gamma=self.config.algorithm.ecvc_gamma,
                            )
                            _delta = _ecvc_diag["delta"]
                        else:
                            _ecvc_diag = None
                        ...
                        if self.global_step >= 1:
                            ...
                            _pos_entropy_cpu = pos_entropy.detach().cpu().float()
                            _neg_entropy_cpu = neg_entropy.detach().cpu().float()
                            _entropy_gap_cpu = (
                                _ecvc_diag["entropy_gap"].detach().cpu().float() if _ecvc_diag is not None else None
                            )
                            _entropy_gap_z_cpu = (
                                _ecvc_diag["entropy_gap_z"].detach().cpu().float() if _ecvc_diag is not None else None
                            )
                            _gate_pos_cpu = (
                                _ecvc_diag["gate_pos"].detach().cpu().float() if _ecvc_diag is not None else None
                            )
                            _gate_neg_cpu = (
                                _ecvc_diag["gate_neg"].detach().cpu().float() if _ecvc_diag is not None else None
                            )
                                ...
                                for _i, _uid in enumerate(_uids):
                                    ...
                                    for _pos in _valid_positions:
                                        ...
                                        _row = {
                                            ...
                                            "vis_teacher_entropy": float(_pos_entropy_cpu[_i, _pos].item()),
                                            "mask_teacher_entropy": float(_neg_entropy_cpu[_i, _pos].item()),
                                            "entropy_gap": (
                                                float(_entropy_gap_cpu[_i, _pos].item())
                                                if _entropy_gap_cpu is not None else None
                                            ),
                                            "entropy_gap_z": (
                                                float(_entropy_gap_z_cpu[_i, _pos].item())
                                                if _entropy_gap_z_cpu is not None else None
                                            ),
                                            "gate_pos": (
                                                float(_gate_pos_cpu[_i, _pos].item())
                                                if _gate_pos_cpu is not None else None
                                            ),
                                            "gate_neg": (
                                                float(_gate_neg_cpu[_i, _pos].item())
                                                if _gate_neg_cpu is not None else None
                                            ),
                                ...
                                json.dump(
                                    {
                                        ...
                                        "cepo_lambda": float(_cepo_lambda_used),
                                        "cepo_eps_w": float(self.config.algorithm.cepo_eps_w),
                                        "ecvc_entropy_gap_std": self._ecvc_entropy_gap_std,
                                    },
                                    _sf,
                                    ensure_ascii=False,
                                    indent=2,
                        ...
                        if _ecvc_diag is not None:
                            metrics["ecvc/sigma_U_init"] = self._ecvc_entropy_gap_std
                            metrics["ecvc/entropy_gap_mean"] = _ecvc_diag["entropy_gap"][_mask].mean().item()
                            metrics["ecvc/entropy_gap_std"] = _ecvc_diag["entropy_gap"][_mask].std().item()
                            metrics["ecvc/entropy_gap_z_mean"] = _ecvc_diag["entropy_gap_z"][_mask].mean().item()
                            metrics["ecvc/gate_pos_mean"] = _ecvc_diag["gate_pos"][_mask].mean().item()
                            metrics["ecvc/gate_neg_mean"] = _ecvc_diag["gate_neg"][_mask].mean().item()
                            metrics["ecvc/visual_evidence_mean"] = _ecvc_diag["visual_evidence"][_mask].mean().item()
                            metrics["ecvc/delta_mean"] = _ecvc_diag["delta"][_mask].mean().item()
