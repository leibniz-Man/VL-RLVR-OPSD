...
class FSDPWorker(Worker):
    def __init__(
        ...
        if self.config.actor.disable_kl and not self.config.actor.cepo_needs_ref:
            self._has_ref = False
    ...
    def _process_multi_modal_inputs_from_key(self, data: DataProto, source_key: str, target_key: str):
        ...
        video_fps = data.meta_info["video_fps"]
        batch_multi_modal_inputs = []
        for multi_modal_data in data.non_tensor_batch[source_key]:
            images, videos = [], []
            if "images" in multi_modal_data:
                for image in multi_modal_data["images"]:
                    images.append(process_image(image, min_pixels, max_pixels))
        ...
        data.non_tensor_batch[target_key] = np.array(batch_multi_modal_inputs, dtype=object)
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_cepo_teacher_log_probs(self, data: DataProto):
        teacher_src = "ref" if self.config.actor.cepo_needs_ref else "actor"


        if teacher_src == "ref":
            assert self._has_ref, "cepo_teacher_source='ref' requires _has_ref"
            module   = self.ref_fsdp_module
            policy   = self.ref_policy
            offload  = self._use_ref_param_offload or (self._is_lora and self._use_param_offload)
            adapter_ctx = self.ref_fsdp_module.disable_adapter() if self._is_lora else nullcontext()
        else:
            assert self._has_actor
            module   = self.fsdp_module
            policy   = self.actor
            offload  = self._use_param_offload
            adapter_ctx = nullcontext()

        self._process_multi_modal_inputs(data)
        self._process_multi_modal_inputs_from_key(data, "pos_multi_modal_data", "pos_multi_modal_inputs")
        self._process_multi_modal_inputs_from_key(data, "neg_multi_modal_data", "neg_multi_modal_inputs")
        data = data.to(torch.cuda.current_device())

        if offload:
            load_fsdp_model(module)

        data.meta_info["temperature"] = self.config.rollout.temperature
        with self.ulysses_sharding_manager, adapter_ctx:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            pos_lp, neg_lp, pos_entropy, neg_entropy = policy.compute_cepo_log_probs(data=data)
            output = DataProto.from_dict(tensors={
                "pos_teacher_log_probs": pos_lp,
                "neg_teacher_log_probs": neg_lp,
                "pos_teacher_entropies": pos_entropy,
                "neg_teacher_entropies": neg_entropy,
            })
            output = self.ulysses_sharding_manager.postprocess_data(output)
    ...
    def compute_teacher_log_probs(self, data: DataProto):
        """Compute teacher-mode log probs for RLSD token-level credit assignment.

        Uses the SAME actor weights as the student, but with teacher inputs:
        [prompt + ground-truth answer + response] instead of [prompt + response].
        The privileged answer text shifts each token's log prob — this delta
        (stop-gradiented) becomes the per-token credit weight in RLSD.

        Output key is 'teacher_log_probs', parallel to 'old_log_probs' from
        compute_log_probs. Called once per training step, right after
        compute_log_probs, before advantage computation.
        """
        assert self._has_actor

        # Images are identical to the student batch — _process_multi_modal_inputs
        # will hit the cache set by the preceding compute_log_probs call.
        self._process_multi_modal_inputs(data)
        data = data.to(torch.cuda.current_device())
        ...
        # Unshard root FSDP module — same as compute_log_probs
        if self.world_size > 1:
            self.fsdp_module._handle.reshard(True)
